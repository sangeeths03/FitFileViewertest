...
function patchSummaryFields(obj) {
    ...
    // Distance
    if (obj.total_distance != null) obj.total_distance = formatDistance(Number(obj.total_distance));
    if (obj.totalDistance != null) obj.totalDistance = formatDistance(Number(obj.totalDistance));
    // Time
    ...
    function formatArray(val, digits = 2) {
        ...
        if (typeof val === 'string' && val.includes(',')) return val.split(',').map(v => Number(v).toFixed(digits)).join(", ");
        return val;
    }
